...
def query_a(film_rdd):
    ...
    return film_rdd.filter(lambda row: row.rating == 'PG').map(lambda row: (row.title, row.description))
...
def query_b(film_rdd):
    ...
    """
    # Implement the RDD transformation and action here
    return (film_rdd.filter(lambda row: row.length >= 60)
            .map(lambda row: (row.rating, (row.replacement_cost, 1)))
            .reduceByKey(lambda a, b: (a[0] + b[0], a[1] + b[1]))
            .filter(lambda x: x[1][1] >= 160)
...
def query_c(film_actor_rdd):
    ...
    """
    # Implement the RDD transformation and action here
    actors_in_film_1_rdd = film_actor_rdd.filter(lambda row: row.film_id == 1).map(lambda row: row.actor_id)
    actors_in_film_23_rdd = film_actor_rdd.filter(lambda row: row.film_id == 23).map(lambda row: row.actor_id)
    return actors_in_film_1_rdd.intersection(actors_in_film_23_rdd)
...
def query_d(actor_rdd, film_actor_rdd):
    ...
    # Implement the RDD transformation and action here
    return (actor_rdd
            .map(lambda row: (row.actor_id, (row.first_name, row.last_name)))
            .join(film_actor_rdd.map(lambda row: (row.actor_id, row.film_id)))
            .filter(lambda x: x[1][1] in [1, 2, 3])
            .map(lambda x: x[1][0])
            .distinct()
            .sortBy(lambda x: x[0]))
...
def query_e(film_rdd):
    ...
    # Implement the RDD transformation and action here
    return (film_rdd
            .map(lambda row: ((row.rental_duration, row.rating), (row.length, row.length, row.length, 1)))
            .reduceByKey(lambda a, b: (min(a[0], b[0]), max(a[1], b[1]), a[2] + b[2], a[3] + b[3]))
            .map(lambda x: (x[0][0], x[0][1], x[1][0], x[1][1], x[1][2] / x[1][3], x[1][3]))
